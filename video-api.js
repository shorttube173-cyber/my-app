...
const ShortTubeVideoAPI = {
  // mode: 'home' | 'search'
  async getFeed({ mode = 'home', query = '', page = 1, pageToken = '', category = null } = {}) {
    const cacheKey = `${mode}:${query}:${page}:${category || ''}`;
    const cached = getCached(cacheKey);
    if (cached) return cached;

    const [ytResult] = await Promise.allSettled(
      mode === 'search'
        ? [fetchYouTubeSearch(query, pageToken)]
        : [fetchYouTubeHome(page, category)]
    );
    ...
    const merged = vaShuffle([...yt.items]).filter(v => v.title && v.thumbnail);
    const result = {
      videos: merged,
    ...
    if (merged.length > 0) setCached(cacheKey, result);
    return result;
  },
